...
import type { IMessageProps } from "./props";
    ...
    <div className="p-4">
      ...
      <div className="flex">
        <div>
          ...
          <video ref={localVideoRef} className="w-full mb-4 border border-solid border-blue-600" playsInline></video>
          <div>remoteVideoRef</div>
          <video ref={remoteVideoRef} className="w-full border border-solid border-blue-600" playsInline></video>
        </div>
        <div className="p-4">
          ...
          {logMessage.map((item: IMessageProps, index: number) => (
            <div key={index} className={`mb-2 ${item.type === "error" ? "text-red-500" : ""}`}>
              {item.content}
            </div>
          ))}
        </div>
